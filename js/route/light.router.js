...
(function (){
	...
	var prefix = '#!';
	var Hash = window.location.hash ? window.location.hash : decodeURIComponent(window.location.href).replace(/^.*#/g, '#');
	//
	function Route(){
		//设置状态对象
		this.routeState = [];
	...
	Route.prototype = {
         ...
         *  
         */
		add: function(path, callback, beforeCallback){
		...
		start: function(options){
			...
			document.onclick = function pagecl(event){
				...
				if (href.substring(0, 2) == prefix) {
					...
					/*if (that.routeState.beforeCallback) {
						alert(that.routeState.beforeCallback)
						that.routeState.beforeCallback(target);
					}*/
					setTimeout(function() {
						//做location变化 和 回调执行
						that.guide(href.replace(/#!/g, ''), {replace: false, fromClick: true}, target);
					}, 50);
					return false;
				}
				//console.info(that.routeState);
        ...
        */
		guide: function(hash, options, target){
			//更新location
			this.updateLocation(hash, options);
			//执行路由回调
			this.route(hash, options, target);
		},
		/*
		 * 改变浏览器hash
		 */
		...
		//执行路由回调
		route: function(path, options, target){
			//
			...
			for (var i = 0; i < routeState.length; i++) {
				...
				if (path && path.match(routeState[i][reg_path])) {
					if (this.monitorMode == 'popstate') {
						...
						if (target) {
							if (routeState[i].beforeCallback) {
								routeState[i].beforeCallback(target);
							}
						}
						return routeState[i].callback();
					}
